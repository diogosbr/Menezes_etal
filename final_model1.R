...
#' @export
final_model1 <- function(species_name,
                        algorithms = NULL,
                        weight_par = NULL,
  ...
  for (algo in algorithms) {
    ...
    if (scale_models == T) {
      ...
      #mod.cont
      for(i in 1:dim(mod.cont)[3]) {
        stand <-
          function(x) {
            (x - min(mod.cont[[i]][], na.rm = T)) / (max(mod.cont[[i]][], na.rm = T) - min(mod.cont[[i]][], na.rm = T))
          }
        bb <- calc(mod.cont[[i]], stand)
        bb
        if (i == 1) {
          cc = stack(bb)
          names(cc)[i] = names(mod.cont)[i]
        }
        else{
          cc = stack(cc, bb)
          names(cc)[i] = names(mod.cont)[i]
      ...
      #mod.cut
      for(i in 1:dim(mod.cut)[3]) {
        stand <-
          function(x) {
            (x - min(mod.cut[[i]][], na.rm = T)) / (max(mod.cut[[i]][], na.rm = T) - min(mod.cut[[i]][], na.rm = T))
          }
        bb <- calc(mod.cut[[i]], stand)
        bb
        if (i == 1) {
          cc = stack(bb)
          names(cc)[i] = names(mod.cut)[i]
        }
        else{
          cc = stack(cc, bb)
          names(cc)[i] = names(mod.cut)[i]
